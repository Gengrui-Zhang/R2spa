...
# Data Generation
 ...
 GenData <- function (condition, fixed_objects = NULL) {
   ...
   sd_y <- sqrt(0.766) # cor_xm = 0.3
   rel_val <- c(0.34, 0.26, 0.18) # Reliability = 0.9
...
# Run 2000 replications
 ...
 sim_01222024_redcond <- runSimulation(design = DESIGNFACTOR,
                            replications = 2000,
                            generate = GenData,
                            analyse = extract_res,
...
# Summarize the results
 ...
 write_csv(sim_results, "sim_01222024_redcond.csv")

# Plot the results

 sim_plots <- read.csv("sim_01222024_redcond.csv")
 # # Bias
 # sim_plots %>%
