...
In this equation, $\xi_{X}$ and $\xi_{M}$ are two latent predictors and their product $\xi_{X}\xi_{M}$ form the interaction term, where $\alpha$ represents the intercept. The $\zeta$ is the disturbance of regression model with a distribution of $\zeta \sim N(0, \Psi)$ and $\Psi$ is residual variance. The coefficients $\gamma_{1}$ and $\gamma_{2}$ represent the main effects, while $\gamma_{3}$ represents the interaction effect. The dependent variable can be either observed scores $y$ or latent variables $\eta$ composed of observed items. The measurement models for $\xi$ (s) follow the traditional confirmatory factor analysis (CFA) model:
