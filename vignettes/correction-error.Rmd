...
# Using `get_fs(..., corrected_av_efs = TRUE)
fsy2 <- R2spa::get_fs(y, std.lv = TRUE, corrected_av_efs = TRUE)
attr(fsy2, which = "av_efs")
```
...
jac_a2 <- lavaan::lav_func_jacobian_complex(
    \(x) R2spa:::compute_a(x, lavobj = cfa_fit)[2, ],
    coef(cfa_fit)
)
# Correction[1, 1]
sum(diag(lavInspect(cfa_fit, what = "est")$theta %*%
