...
data <- data.frame(
  ...
  Raw_Biases = c(0.02089093, 0.01260763, 0.00935545, 0.01385633, 0.006056654, NaN)
)
...
RMSE_data <- data.frame(
  Type = c("no_adj_rel", "adj_rel", "quadratic", "no_adj_rel_bc", "adj_rel_bc", "quadratic_bc"),
  RMSE = c(0.9342709, 0.5638304, 0.4183884, 0.6196738, 0.2708618, NaN)
)
print(RMSE_data)
...
1. Assuming no sampling errors in the weights

$$\rho = \frac{(a'\lambda)^2}{(a'\lambda)^2 + a\Theta a}$$

2. Accounting for sampling error in the weights

$$\rho = \frac{(a'\lambda)^2 + \lambda'V_{\tilde{a}}\lambda}{(a'\lambda)^2 + a\Theta a + \lambda'V_{\tilde{a}}\lambda + tr(\Theta V_{\tilde{a}})}$$

3. Derived using a quadratic formula

$$\rho = \frac{1\pm\sqrt{1 - 4\theta_{corrected}/\psi}}{2}$$

Derivation for the reliability formula that accounts for sampling errors in the weights:

Given $$E(\tilde{a}=a, y=\lambda\eta+\varepsilon),V(\eta)=1,E(\eta)=0,V(\varepsilon)=\Theta,\eta\perp\varepsilon,$$
...
V(\tilde{a}'y) 
  ...
  \\
  \\
  & Rel = \frac{(a'\lambda)^2 + \lambda'V_{\tilde{a}}\lambda}{(a'\lambda)^2 + a'\Theta a + tr(\Theta V_{\tilde{a}}) + (a'\lambda)^2} \\
