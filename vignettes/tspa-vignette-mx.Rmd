...
matL <- mxMatrix(
    ...
    free = FALSE,
    values = attr(fs_dat, "fsA")[2:1, 2:1],
    name = "L"
)
# Error
matE <- mxMatrix(
    ...
    free = FALSE,
    values = attr(fs_dat, "av_efs")[2:1, 2:1],
    name = "E"
)
```
...
mxStandardizeRAMpaths(tspa_mx_fit)
```

```{r, eval = FALSE}
tspa_mx <-
    R2spa:::tspa_mx_model(fsreg_umx, data = fs_dat,
                          mat_ld = attr(fs_dat, "fsA"),
                          mat_vc = attr(fs_dat, "av_efs"))
```
...
# Factor scores
mod1 <- mirt(dat1, model = 1, itemtype = "Rasch")
mod2 <- mirt(dat2, model = 1, itemtype = "2PL")
