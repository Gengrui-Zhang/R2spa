...
intNames <- paste0(rep(tvalue, each = length(sources)), sources)
 ...
 # dat_comp <- int_prod(dat, var_1 = c("t1", "t2", "t3", "t4"),
 #                 var_2 = c("s1", "s2", "s3", "s4", "s5"),
 #                 prod_name = intNames)
...
df <- GenData(condition = condition, fixed_objects = fixed_objects)
...
fitME <- sem(model, data = df, std.lv = TRUE, estimator = "mlr")
...
intNames <- paste0(rep(lat_x, each = length(lat_m)), lat_m)

df_ind <- indProd(df, var1 = c("x1", "x2", "x3"),
                var2 = c("m1", "m2", "m3"),
...
fitintMC <- sem(mod_ind, data = df_ind, std.lv = FALSE, meanstructure = TRUE)
summary(fitintMC, fit.measure = TRUE, standardized = TRUE)

```

```{r test data generation function}

# Test function
# cor_xm = 0.3, beta_3 = 0.8, rel = 0.7

   # Generate scores for observed items: x1 - x3, m1 - m3
   eta_scores <- rmnorm(50000, mean = c(0, 0), varcov = matrix(c(1, 0.3,
                                    0.3, 1), nrow = 2)) # Factor scores
   # Generate eta scores with interaction
   eta_scores_int <- cbind(eta_scores, eta_scores[,1]*eta_scores[,2])
   delta <- rmnorm(50000, varcov = diag(c(1.32, 0.99, 0.69),
                                  nrow = 6)) # Errors/Residuals of Indicators
   item_scores <- tcrossprod(eta_scores, Lambda) + delta # Item/Indicator scores

   y_scores <- tcrossprod(eta_scores_int, Gamma) + rnorm(N, 0, sqrt(0.766)) # Y scores / DV scores
   
   df <- cbind(y_scores, eta_scores_int)

model ='
        # Structural Model
            Y ~ X + M + XM'

colnames(df) <- c("Y", "X", "M", "XM")

fit <- sem(model = model, data = df)
summary(fit, standardized = TRUE)

# cor_xm = 0.6, beta_3 = 0.75, rel = 0.9

# Generate scores for observed items: x1 - x3, m1 - m3
   eta_scores <- rmnorm(5000, mean = c(0, 0), varcov = matrix(c(1, 0.6,
                                    0.6, 1), nrow = 2)) # Factor scores
   # Generate eta scores with interaction
   eta_scores_int <- cbind(eta_scores, eta_scores[,1]*eta_scores[,2])
   delta <- rmnorm(N, varcov = diag(c(0.34, 0.26, 0.18),
                                  nrow = 6)) # Errors/Residuals of Indicators
   item_scores <- tcrossprod(eta_scores, Lambda) + delta # Item/Indicator scores
...
model_rapi <- "
              tvalue =~ 1*tsum_centered
              sources =~ 1*ssum_centered
              tip =~ 1*tips_sum_centered
              int =~ 1*int_centered

              # Error variance
              tsum_centered ~~ ve1*tsum_centered
              ssum_centered ~~ ve2*ssum_centered
              int_centered ~~ ve3*int_centered
              tips_sum_centered ~~ vey*tips_sum_centered
