...
```{r try with semdplot}
fs_reg <- lm(fs_dat$fs_dem60 ~ fs_dat$fs_ind60, fs_dat)
fs_sem <- sem("fs_dem60 ~ fs_ind60", fs_dat)
summary(fs_reg)
summary(fs_sem)

p <- casewise_residuals(fs_sem)
plot(p,
     x_names = "fs_ind60",
     y_names = "fs_dem60")
```
...
```{r using semdplot package}
fs_sem2 <- sem(model = "fs_dem60 ~ fs_ind60
                        fs_dem65 ~ fs_ind60 + fs_dem60",
               data = fs_dat)

fit_res2 <- casewise_residuals(fs_sem2)
p <- plot(fit_res2,
           x_names = c("fs_ind60", "fs_dem60"),
           y_names = c("fs_dem60", "fs_dem65"))
